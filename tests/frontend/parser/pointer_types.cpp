...
TEST(Frontend, Parse_Simple_Pointer) {
    std::vector<Token> tokens = {
        ...
        { int_type, "test.basalt", 1, 2, 2, Token::Type::type   },
    };
    ...
    EXPECT_EQ(type.get<PointerType>().pointed_type.get<PrimitiveType>().type_name, int_type);
}
...
TEST(Frontend, Parse_Pointer_To_Pointer) {
    ...
    EXPECT_EQ(inner_pointer.pointed_type.get<PrimitiveType>().type_name, int_type);
}
